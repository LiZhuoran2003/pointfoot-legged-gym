...
import legged_gym.utils.logger
...
class WebViewer:
    def __init__(self, logger: legged_gym.utils.logger.Logger = None, host: str = "127.0.0.1", port: int = 5000) -> None:
        """
        ...
        self._app.add_url_rule("/_route_stream_state_log", view_func=self._route_stream_state_log)
    ...
    def _route_index(self) -> 'flask.Response':
        ...
        # plot_url = self.test_matplot_figure()
        # return flask.render_template_string(template, plot_url=plot_url)
    ...
    def _route_input_event(self) -> 'flask.Response':
        ...
        print(f"Received Action: key: {key}, mouse: {mouse}")
    ...
    def _route_stream_state_log(self) -> 'flask.Response':
        return flask.Response(self._stream_state_log(), mimetype='multipart/x-mixed-replace; boundary=frame')
    
    def _stream(self, stop_state_log=False) -> bytes:
        """Format the image to be streamed

        :return: Image encoded as Content-Type
        :rtype: bytes
        """
        print("_stream thread created!!")
    ...
    def _stream_state_log(self) -> bytes:
        print("_stream_state_log thread created!!")
        while True:
            self._event_stream_state_log.wait()
            print("_stream_state_log thread triggered!!")

            # prepare image
            try:
                image_path = os.path.join(LEGGED_GYM_ROOT_DIR, "legged_gym", "utils", "state_log.jpeg")
                ...
                print("state_log.jpeg file not found.")
                            
            self._event_stream_state_log.clear()
    ...
    def render(self,
               ...
               stop_state_log: bool = False) -> None:
        ...
        # if stop_state_log:    # ABORTED.
        #     self.logger.plot_states_on_web()
        #     self._event_stream_state_log.set()
